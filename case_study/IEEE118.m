...
dt=0.001;tf=5;
tspan=dt:dt:tf;
f= @(x) power_func(x);% full order model
xdotNL= @(t,x) f(x);
...
disp('Snapshots recorded')
 ...
 %[tFOM,xROM]=implicitEuler(xdotr,tspan,x0r);
 ROM_time=toc
 x=xROM;
...
ylabel('Latent State','Interpreter','latex')
grid on
...
xlabel('$t$','Interpreter','latex')
ylabel('Avg. $\omega$','Interpreter','latex')
grid on
title(['IEEE', num2str(size(mpc.bus,1)), '-Bus System', ' SINDy Model'],'Interpreter','latex')
set(gca,'ticklabelinterpreter','latex','Fontsize',15)
%% predicted model
figure(3);
subplot(2,1,1)
...
for i=1:12
    ...
    xlabel('$t[s]$','Interpreter','latex')
    ylabel(['Latent State ', num2str(i)],'Interpreter','latex')
    set(gca,'ticklabelinterpreter','latex','Fontsize',15)
    grid on
    xlim([0,5])
end
